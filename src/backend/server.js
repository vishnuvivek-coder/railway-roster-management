...
// Prototype: Fast Persona Switcher for Demonstrations & Stakeholders
app.post('/api/auth/demo-switch', async (req, res) => {
  ...
  try {
    ...
    let targetName = 'Chief Depot Officer (Admin)';
    let targetStaffName = null;
    let targetCategoryCode = 'COR';

    if (personaKey === 'employee1_cor') {
      targetUsername = 'employee1_cor';
      targetRole = 'Staff';
      targetName = 'Employee 1 (COR Conductor)';
      targetStaffName = 'Employee 1';
      targetCategoryCode = 'COR';
    } else if (personaKey === 'employee5_sleeper') {
      targetUsername = 'employee5_sleeper';
      targetRole = 'Staff';
      targetName = 'Employee 5 (Sleeper Staff)';
      targetStaffName = 'Employee 5';
      targetCategoryCode = 'SLEEPER';
    } else if (personaKey === 'employee1_ladies') {
      targetUsername = 'employee1_ladies';
      targetRole = 'Staff';
      targetName = 'Employee 1 (Ladies Staff)';
      targetStaffName = 'Employee 1';
      targetCategoryCode = 'LADIES';
    } else if (personaKey === 'pending_user') {
      targetUsername = 'new_conductor_applicant';
      targetRole = 'Staff';
      targetName = 'New Conductor (Pending Verification)';
    }

    // Find staff if targetStaffName specified
    let staffRecord = null;
    if (targetStaffName) {
      const cat = await get('SELECT id FROM categories WHERE code = ?', [targetCategoryCode]);
      if (cat) {
        staffRecord = await get('SELECT * FROM staff WHERE category_id = ? AND name = ?', [cat.id, targetStaffName]);
      }
      if (!staffRecord) {
        staffRecord = await get('SELECT * FROM staff WHERE name = ? LIMIT 1', [targetStaffName]);
      }
    }

    // Ensure user exists in users table
    let user = await get('SELECT * FROM users WHERE username = ?', [targetUsername]);
    if (!user) {
      const hash = await bcrypt.hash('demo123', 10);
      const isPending = personaKey === 'pending_user';
      const insertRes = await run(
        ...
        [
          targetUsername,
          targetName,
          `${targetUsername}@railway.gov`,
          hash,
          targetRole,
          isPending ? 'PENDING' : 'APPROVED',
          staffRecord ? staffRecord.id : null,
          isPending ? null : new Date().toISOString(),
          isPending ? null : 'System'
        ]
      );
      user = await get('SELECT * FROM users WHERE id = ?', [insertRes.lastID]);
    } else if (staffRecord && user.staff_id !== staffRecord.id) {
      await run('UPDATE users SET staff_id = ? WHERE id = ?', [staffRecord.id, user.id]);
      user.staff_id = staffRecord.id;
    }

    if (user.status !== 'APPROVED') {
      return res.status(403).json({
        error: 'This demo persona is currently in PENDING status to demonstrate the Master Admin user verification workflow.',
        user: {
          id: user.id,
          username: user.username,
          name: user.name,
    ...
    const token = jwt.sign(
      { id: user.id, username: user.username, role: user.role, name: user.name, staff_id: user.staff_id },
      JWT_SECRET,
      { expiresIn: '7d' }
    );
